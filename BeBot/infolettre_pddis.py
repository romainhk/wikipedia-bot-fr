...
class Infolettre:
    ...
    def __init__(self, site, infolettre, debug):
        ...
        self.abn = {
                "wikimag" : "Wikipédia:Wikimag/Abonnement",
                "raw"     : "Wikipédia:Regards sur l'actualité de la Wikimedia/Inscription" }
    ...
    def wikimag(self):
        """ Wikimag """
        mag = pywikibot.Page(self.site, 'Wikipédia:Wikimag/%s/%s' % (self.annee, self.semaine) )
        if not mag.exists():
        ...
        numero = '???'
        num = re.compile("\|numéro *= *(\d+)", re.LOCALE|re.UNICODE)
        m = num.search(mag.text)
        if m is not None:
            numero = m.group(1)

        self.resume = 'Demandez [[Wikipédia:Wikimag/%s/%s|Cannes Midi (n°%s)]]. Le tueur de Cannes frappe encore... 5 cents' \
                % (self.annee, self.semaine, numero)
        # Message à distribuer
        msg  = "\n\n== Wikimag n°%s - Semaine %s ==\n" % (numero, self.semaine)
        msg += "{{Wikimag message|%s|%s|%s}} ~~~~" % (numero, self.annee, self.semaine)
    ...
    def raw(self):
        ...
        self.resume = "Regards sur l'actualité de la Wikimedia - semaine %s de %s" % (self.semaine, self.annee)
        msg  = "\n\n== Regards sur l'actualité de la Wikimedia - Semaine %s ==\n" % (self.semaine)
        msg += "{{Regards sur l'actualité de la Wikimedia/PdD|%s|%s}}\n~~~~" % (self.annee, self.semaine)
        return msg
