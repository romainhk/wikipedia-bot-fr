...
class Trad_maintenance:
    ...
    def __init__(self, site, debug):
        self.site = site
        self.debug = debug
        self.resume = 'Restructuration du [[Projet:Traduction]]'
        ...
        self.les_statuts = [ 'Demandes', 'En cours', 'A relire', 'En relecture', 'Terminée' ]
        les_mois = { 'janvier' : 0,    'février' : 0,     'mars' : 0,        'avril' : 0,
    ...
    def __str__(self):
        ...
        r += '* Nb de pages supprimées: %i\n' % self.stats['suppr']
        r += '* Nb de traductions closes: %i\n' % self.stats['cloture']
        r += '* Nb de {{Traduit de}} ajoutés: %i\n' % self.stats['traduitde']
    ...
    def run(self):
        if self.debug:
            pywikibot.warning("== Mode débuggage actif ; aucunes modifications effectuées ==")
        parannee = pywikibot.Category(self.site, "Catégorie:Traduction par année")
