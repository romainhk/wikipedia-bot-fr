...
class ListageQualite:
    ...
    def __init__(self, site):
        ...
        if sous_page.has_key(self.langue):
            self.page_projet = wikipedia.Page(self.site, \
    ...
    def lycos(self, nom_base, conditions=None):
        ...
        for a in articles:
            page = {}
            nom_page = html2unicode(a[0])
            page[champs[1]] = int(a[1])
            if a[2]:
                page[champs[2]] = html2unicode(a[2])
            else:
                page[champs[2]] = None
            if a[3]:
                page[champs[3]] = html2unicode(a[3])
            else:
                page[champs[3]] = None
            rep[nom_page] = page
        return rep
...
def main():
    ...
    for cl in wikis:
        ...
        lq = ListageQualite(wikipedia.Site(cl))
        lq.run()
        log += unicode(lq)
        lq.publier()
    #wikipedia.Page(wikipedia.Site('fr'), u'Utilisateur:BeBot/Listage qualité').put(log, \
